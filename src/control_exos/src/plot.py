...
class ImprovedEllipseTrajectoryNode(Node):
    def __init__(self):
        ...
        self.y_offset = 760.0   # Độ cao cơ sở
         ...
         # Thêm các tham số điều chỉnh tốc độ
        self.declare_parameter('max_velocity', 2.0)
        self.declare_parameter('max_acceleration', 1.5)
        self.declare_parameter('step_size', 10.0)
        ...
        # Thông số điều khiển
        self.declare_parameter('control_frequency', 10000.0)  # 50Hz
        self.declare_parameter('movement_duration', 0.08)  # 0.1s cho mỗi điểm
    ...
    def move_leg_smooth(self, leg_name, target_positions):
        ...
        # Thêm vận tốc tối đa cho các khớp
        max_velocity = 2.0  # radians/second
        max_acceleration = 1.5
        #point.velocities = [max_velocity] * len(target_positions)  
        
        # Thêm gia tốc tối đa cho các khớp
        #max_acceleration = 1.5  # radians/second^2
        smoothing_factor = 1.1
    
        point.velocities = [self.max_velocity * smoothing_factor] * len(target_positions)
        point.accelerations = [self.max_acceleration * smoothing_factor] * len(target_positions)
